...
\fillhere{
Misinformation is a big problem in today's world. One specific type of misinformation that stands out is conspiracy theories. A conspiracy theory is a piece of text that contains very radical
conclusions which aren't built on a rational basis. An interesting problem regarding conspiracy theories in NLP is: "Can conspiracy theories be characterized only by their semantic structure?" At
first glance , it appears not, but there exists research such as \citep{fort2023bigfoot} that makes this a promising path to take. This is also a relevant problem since conspiracy theories evolve
according to the current state of the world. It would be very beneficial to produce a model that can classify a conspiracy theory only based on its semantic structure. Since we plan on using modern
methods of LLMs, a question of pretraining data also arises. Will our model work better if we additionally pretrain it on a conspiracy theory corpus?
}
...
\noindent\textbf{RQ1:} 
\fillhere{
  To what extent do modern LLMs utilize emergent reasoning to identify the semantic structure of conspiracy theories when all context dependent entities are dynamically masked?}
\textit{Contribution:}
...
\textit{Contribution:} \fillhere{
This doesn't seem too hard to test, but it would give some results that could be
useful beyond computation social science.
}
...
\textit{Contribution:} \fillhere{
We haven't found any research that deals with this so it would be an interesting problem.
But we should investigate other work a bit more because there probably is something.
}
...
\fillhere{LOCO (description of LOCO)}
...
\fillhere{some model that is easy to additionaly pretrain. \citep{fort2023bigfoot} paper is our baseline}
...
\fillhere{For sure the same metrics as \citep{fort2023bigfoot} (so we can compare results), but there should be some discussion regarding evaluation on other tasks}
...
\fillhere{You can write something about masking}
...
\fillhere{I am sure you can think of something.}
